...
var videos = {
	...
	loading: function(f){
		if(f == 'start'){
			...
			'<div class="video_show_object" style="padding-top:230px;height:230px"><center><img src="/templates/Default/images/progress_gray.gif" alt="" /></center></div><div class="video_show_panel" style="height:20px"></div>'+
			'</div>'+
			'</div>';
			$('body').append(loadcontent);
			$('#video_load').show();
...
var wall = {
	...
	event: function(event){
		...
		if(fast_oi != 'fast_form' && fast_oi != 'fast_link' && fast_oi != 'fast_inpt' && fast_oi != 'fast_text' && fast_oi != 'fast_buts')
	...
	send: function(){
		...
		if(wall_text != 0 || attach_files != 0){
			...
			$.post('/index.php?go=wall&act=send', {wall_text: wall_text, for_user_id: for_user_id[1], attach_files: attach_files, vote_title: $('#vote_title').val(), vote_answer_1: $('#vote_answer_1').val(), vote_answer_2: $('#vote_answer_2').val(), vote_answer_3: $('#vote_answer_3').val(), vote_answer_4: $('#vote_answer_4').val(), vote_answer_5: $('#vote_answer_5').val(), vote_answer_6: $('#vote_answer_6').val(), vote_answer_7: $('#vote_answer_7').val(), vote_answer_8: $('#vote_answer_8').val(), vote_answer_9: $('#vote_answer_9').val(), vote_answer_10: $('#vote_answer_10').val()}, function(data){
				if(data == 'err_privacy'){
					addAllErr(lang_pr_no_title);
				} else {
	...
	attach_insert: function(type, data, action_url, uid){
		...
		if(type == 'audio'){
			...
			$('#attach_files').append('<span id="attach_file_'+res_attach_id+'" class="attach_file fl_l" style="display:block;width:100%"><div class="audio_wall_attach"><div class="fl_l"><b>'+artist+'</b> &ndash; '+name+'</div><img src="/templates/Default/images/close_a.png" onMouseOver="myhtml.title(\''+res_attach_id+'\', \''+lang_wall_no_atttach+'\', \'wall_audio_\')" onClick="wall.attach_delete(\''+res_attach_id+'\', \'audio|'+action_url+'||\')" id="wall_audio_'+res_attach_id+'" class="fl_l cursor_pointer" style="margin-left:5px;margin-top:1px" /></span></div>');
...
var gStatus = {
	...
	set: function(clear){
		...
		if(text != $('#new_status').text()){
			...
			$.post('/index.php?go=status', {text: text}, function(data){
				if(data){
					$('#status_link').hide();
					gStatus.tell();
	...
	startTell: function(){
		...
		if(tell_friends){
			if(text != 0){
				$.post('/index.php?go=wall&act=send', {wall_text: text, for_user_id: for_user_id[1]}, function(data){
					$('#wall_records').html(data);
					$('#wall_all_record').html('');
